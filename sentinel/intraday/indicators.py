...
def calculate_intraday_win_rates(
    session: Session, lookback_days: int = 180, gain_threshold: float = 0.05, min_samples: int = 5
) -> int:
    """
    ...
    for stock in stocks:
        ...
        prices_df["daily_return"] = (prices_df["close"] - prices_df["prev_close"]) / prices_df[
            "prev_close"
        ]
        ...
        signals = prices_df[
            (prices_df["daily_return"] >= gain_threshold) & (prices_df["next_open"].notnull())
        ]
        ...
        indicator = session.get(
            IntradayIndicator, (stock.market, stock.symbol, "overnight_win_rate")
        )
        if not indicator:
            indicator = IntradayIndicator(
                market=stock.market, symbol=stock.symbol, indicator_name="overnight_win_rate"
            )
