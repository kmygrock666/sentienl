...
def run_tomorrow_star_scan(
    ...
    target_stocks_stmt = (
        ...
        .where(
            and_(
                ...
                func.length(DailyPrice.symbol) == 4
            )
        )
    ...
    subq_snap = (
        ...
        .where(
            and_(
                IntradaySnapshot.trading_date == today,
                IntradaySnapshot.snapshot_time <= "12:00"
            )
        )
        .subquery()
    )
    snap_stmt = select(subq_snap.c.market, subq_snap.c.symbol, subq_snap.c.cumulative_volume).where(subq_snap.c.rn == 1)
    
    snapshots = { (s.market, s.symbol): s.cumulative_volume for s in session.execute(snap_stmt).all() }
    
    # 5. Load historical Win Rates (Phase 1 indicators)
    ind_stmt = select(IntradayIndicator).where(IntradayIndicator.indicator_name == "overnight_win_rate")
    ...
    for msg in raw_msgs:
        ...
        if gain < 0.03 or p["close"] <= p["open"]:
            continue
        ...
        if p["last_trade_volume"] > 0 and locked_vol > 0:
            if p["last_trade_volume"] >= (locked_vol / 3.0):
                is_great_power = True
