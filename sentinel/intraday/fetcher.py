...
class MISFetcher:
    ...
    def fetch_batch(self, symbols: List[str], markets: List[str], max_retries: int = 3) -> List[Dict[str, Any]]:
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    if is_html:
                        logger.error(f"MIS API returned real HTML instead of JSON. Body snippet: {response.text[:200]}")
                    raise
                
                if "msgArray" in data:
                    return data["msgArray"]
                else:
                    logger.warning(f"MIS API returned no data (Attempt {attempt+1}): {ex_ch[:50]}...")
    ...
    def fetch_all(self, symbols: List[str], markets: List[str], batch_size: int = 50) -> List[Dict[str, Any]]:
