...
def update_intraday_trades(
    session: Session, real_time: bool = False, price_type: str = "open", allow_today: bool = False
) -> int:
    """
    ...
    for trade in open_trades:
        ...
        if exit_price and exit_price > 0:
            ...
            trade.profit_loss = (trade.exit_price - float(trade.entry_price)) / float(
                trade.entry_price
            )
            trade.status = "closed"
            count += 1
...
def monitor_and_close_intraday_trades(
    session: Session, threshold: float = 0.02, force_close: bool = False, allow_today: bool = False
) -> int:
    """
    ...
    trades_to_check = (
        open_trades if allow_today else [t for t in open_trades if t.entry_date < today]
    )
    ...
    for trade in trades_to_check:
        ...
        if should_close:
            ...
            count += 1
            logger.info(
                f"Closing {trade.symbol} at {current_price} (P/L: {pl_ratio:.2%}) - Reason: {reason}"
            )
...
def add_manual_intraday_trade(
    session: Session, market: str, symbol: str, entry_price: float, notes: str = None
) -> bool:
    """
    ...
    if market is None:
        ...
        if len(stocks) == 0:
            ...
            logger.error(
                f"Cannot add trade: Stock {symbol} found in multiple markets ({markets_found}). Please specify --market."
            )
            return False
