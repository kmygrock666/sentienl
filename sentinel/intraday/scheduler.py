...
class IntradayScheduler:
    ...
    def is_trading_day(self, session: Session, check_date: date) -> bool:
        ...
        stmt = select(TradingCalendar.is_trading_day).where(
            and_(TradingCalendar.exchange == "TWSE", TradingCalendar.calendar_date == check_date)
        )
    ...
    def run_trade_settlement(self, force_close: bool = False):
        ...
        def job(session):
            count = monitor_and_close_intraday_trades(
                session, threshold=0.02, force_close=force_close
            )
    ...
    def start(self):
        ...
        logger.info(
            "Scheduled: 09:00-09:29 (SL/TP Monitor), 09:30 (Cutoff), 12:00 (Baseline Snapshot), 13:00 (Tomorrow's Star Strategy)"
        )
